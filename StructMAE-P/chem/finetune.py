...
def main():
    ...
    args.use_early_stopping = args.dataset in ("muv", "toxcast", "tox21")
    ...
    if args.use_early_stopping:
        fw.write(f"early stop:  val: {best_val_acc}, test: {final_test_acc}\n")
    else:
        fw.write(f"no earlystop:  val: {val_acc_list[-1]}, test: {test_acc_list[-1]}\n")
    fw.write(f"\n\n")
    fw.close()
