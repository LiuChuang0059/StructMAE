...
class MaskAtom:
    def __init__(self, num_atom_type, num_edge_type, mask_rate, alpha, max_epoch, predefine, mask_edge=True):
        """
    ...
    def __call__(self, data, epoch, masked_atom_indices=None):
        """
        ...
        * mask rate number of atom indices
        ...
        if masked_atom_indices == None:
            ...
            if self.predefine == "pagerank":
                ...
                raise ValueError("No such predefine metric!")
            tmp_mask_rate = self.mask_rate * np.sqrt(epoch / self.max_epoch)
            batch = torch.zeros(num_atoms, dtype=torch.int64).to(data.x.device)
            throw_nodes_first = topk(scores, tmp_mask_rate, batch).to(data.x.device)  # 选取得分最高的关注节点
            tmp_scores = torch.tensor(np.random.uniform(0, 1, num_atoms)).to(data.x.device)  # 生成随机分数
            tmp_scores[throw_nodes_first] += self.alpha  # 将关注节点的分数加上alpha
            masked_atom_indices = topk(tmp_scores, self.mask_rate, batch).to(data.x.device) # mask得分最高的mask_rate
