...
def main():
    ...
    parser.add_argument("--predefine", type=str, default="pagerank")
    ...
    # 额外传入epoch数作为参数
    loader = DataLoaderMaskingPred(dataset, alpha=args.alpha, max_epoch=args.epochs, batch_size=args.batch_size, shuffle=True, num_workers = args.num_workers, mask_rate=args.mask_rate, mask_edge=args.mask_edge, predefine=args.predefine)
    ...
    # 在这里修改输出模型的路径
    output_file_temp = f"./checkpoints/predefine_{args.gnn_type}_{args.mask_rate}_{args.alpha}.txt"
    ...
    for epoch in range(1, args.epochs+1):
        ...
        # 在这里输出每个epoch的loss，以检验是否已经收敛。最好写在文本文件中。
        print(train_loss)
        # debug_file.write(f"{train_loss}\n")
