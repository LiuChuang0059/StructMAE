...
def pretrain(model, pooler, dataloaders, optimizer, max_epoch, device, scheduler, num_classes, lr_f, weight_decay_f,
    ...
    for epoch in epoch_iter:
        ...
        for batch in train_loader:
            ...
            feat = batch_g.x
            model.train()
            start_learning_epoch = int(args.start_rate * max_epoch)
            loss, loss_dict = model(feat, batch_g.edge_index, batch_g.batch, epoch, max_epoch, args.curMode, start_learning_epoch)  # 把批次信息传进去
...
if __name__ == "__main__":
    ...
    print(args)
    file_name = f"learnable_{args.dataset}.txt"
    main(args)